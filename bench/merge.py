...
by (session_id, agent_id): the latest valid `ts` wins, and a completed/legacy
row wins only when timestamps tie (the same recorded-history rule used by
bench/dashboard.py, /api/rows, /api/export.* and SSE). Each row is tagged with a
